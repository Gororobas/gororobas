...
// This file was autogenerated by `migrate.ts` based on `src/db/migrations-sql/20260822184548_initial.sql`
import { SqlClient } from "effect/unstable/sql"

export default Effect.gen(function* () {
  const sql = yield* SqlClient.SqlClient
  yield* sql`-- Create "accounts" table
CREATE TABLE \`accounts\` (
  ...
  yield* sql`-- Create index "accounts_email" to table: "accounts"
CREATE UNIQUE INDEX \`accounts_email\` ON \`accounts\` (\`email\`)`

  yield* sql`-- Create "sessions" table
CREATE TABLE \`sessions\` (
  ...
  yield* sql`-- Create index "sessions_token" to table: "sessions"
CREATE UNIQUE INDEX \`sessions_token\` ON \`sessions\` (\`token\`)`

  yield* sql`-- Create index "session_account_id_idx" to table: "sessions"
CREATE INDEX \`session_account_id_idx\` ON \`sessions\` (\`account_id\`)`

  yield* sql`-- Create "oauth_accounts" table
CREATE TABLE \`oauth_accounts\` (
  ...
  yield* sql`-- Create index "account_account_id_idx" to table: "oauth_accounts"
CREATE INDEX \`account_account_id_idx\` ON \`oauth_accounts\` (\`account_id\`)`

  yield* sql`-- Create "verifications" table
CREATE TABLE \`verifications\` (
  ...
  yield* sql`-- Create index "verification_identifier_idx" to table: "verifications"
CREATE INDEX \`verification_identifier_idx\` ON \`verifications\` (\`identifier\`)`

  yield* sql`-- Create "profiles" table
CREATE TABLE \`profiles\` (
  ...
  yield* sql`-- Create index "profiles_handle" to table: "profiles"
CREATE UNIQUE INDEX \`profiles_handle\` ON \`profiles\` (\`handle\`)`

  yield* sql`-- Create index "idx_profiles_handle" to table: "profiles"
CREATE INDEX \`idx_profiles_handle\` ON \`profiles\` (\`handle\`)`

  yield* sql`-- Create "people" table
CREATE TABLE \`people\` (
  \`id\` text NULL,
  ...
  yield* sql`-- Create "organizations" table
CREATE TABLE \`organizations\` (
  ...
  yield* sql`-- Create "organization_memberships" table
CREATE TABLE \`organization_memberships\` (
  ...
  yield* sql`-- Create "organization_invitations" table
CREATE TABLE \`organization_invitations\` (
  ...
  yield* sql`-- Create index "organization_invitations_organization_id_email" to table: "organization_invitations"
...
CREATE INDEX \`idx_tags_handle\` ON \`tags\` (\`handle\`)`
  ...
  yield* sql`-- Create "suggested_tags" table
CREATE TABLE \`suggested_tags\` (
  ...
  yield* sql`-- Create index "suggested_tags_handle" to table: "suggested_tags"
CREATE UNIQUE INDEX \`suggested_tags_handle\` ON \`suggested_tags\` (\`handle\`)`

  yield* sql`-- Create "suggested_tag_sources" table
...
CREATE TABLE \`image_credits\` (
  ...
  yield* sql`-- Create "wiki_article_crdts" table
CREATE TABLE \`wiki_article_crdts\` (
  ...
  yield* sql`-- Create "wiki_article_revisions" table
CREATE TABLE \`wiki_article_revisions\` (
  ...
  yield* sql`-- Create index "idx_wiki_article_revisions_article_evaluation" to table: "wiki_article_revisions"
CREATE INDEX \`idx_wiki_article_revisions_article_evaluation\` ON \`wiki_article_revisions\` (\`wiki_article_id\`, \`evaluation\`)`

  yield* sql`-- Create "wiki_articles" table
CREATE TABLE \`wiki_articles\` (
  ...
  yield* sql`-- Create index "idx_wiki_articles_kind_status" to table: "wiki_articles"
CREATE INDEX \`idx_wiki_articles_kind_status\` ON \`wiki_articles\` (\`kind\`, \`status\`)`

  yield* sql`-- Create "wiki_article_translations" table
CREATE TABLE \`wiki_article_translations\` (
  ...
  yield* sql`-- Create "wiki_article_handle_owners" table
CREATE TABLE \`wiki_article_handle_owners\` (
  ...
  yield* sql`-- Create index "wiki_article_handle_owners_wiki_article_id_kind_handle" to table: "wiki_article_handle_owners"
CREATE UNIQUE INDEX \`wiki_article_handle_owners_wiki_article_id_kind_handle\` ON \`wiki_article_handle_owners\` (\`wiki_article_id\`, \`kind\`, \`handle\`)`
  ...
  yield* sql`-- Create "wiki_article_translation_handles" table
CREATE TABLE \`wiki_article_translation_handles\` (
  ...
  yield* sql`-- Create "vegetable_crdts" table
CREATE TABLE \`vegetable_crdts\` (
  ...
  yield* sql`-- Create "vegetable_revisions" table
...
CREATE TABLE \`vegetables\` (
  ...
  yield* sql`-- Create index "vegetables_handle" to table: "vegetables"
CREATE UNIQUE INDEX \`vegetables_handle\` ON \`vegetables\` (\`handle\`)`

  yield* sql`-- Create index "idx_vegetables_handle" to table: "vegetables"
...
CREATE INDEX \`idx_vegetable_varieties_handle\` ON \`vegetable_varieties\` (\`handle\`)`
  ...
  yield* sql`-- Create "vegetable_variety_translations" table
...
CREATE TABLE \`vegetable_photo_metadata\` (
  ...
  yield* sql`-- Create "resource_crdts" table
CREATE TABLE \`resource_crdts\` (
  ...
  yield* sql`-- Create "resource_revisions" table
...
CREATE TABLE \`resource_vegetables\` (
  ...
  yield* sql`-- Create "publication_crdts" table
CREATE TABLE \`publication_crdts\` (
  ...
  yield* sql`-- Create "publication_commits" table
CREATE TABLE \`publication_commits\` (
  ...
  yield* sql`-- Create "publications" table
CREATE TABLE \`publications\` (
  ...
  yield* sql`-- Create index "publications_handle" to table: "publications"
CREATE UNIQUE INDEX \`publications_handle\` ON \`publications\` (\`handle\`)`

  yield* sql`-- Create index "idx_publications_handle" to table: "publications"
CREATE INDEX \`idx_publications_handle\` ON \`publications\` (\`handle\`)`

  yield* sql`-- Create index "idx_publications_kind" to table: "publications"
CREATE INDEX \`idx_publications_kind\` ON \`publications\` (\`kind\`)`

  yield* sql`-- Create "publication_translations" table
CREATE TABLE \`publication_translations\` (
  ...
  yield* sql`-- Create "publication_tags" table
CREATE TABLE \`publication_tags\` (
  ...
  yield* sql`-- Create "publication_vegetables" table
CREATE TABLE \`publication_vegetables\` (
  ...
  yield* sql`-- Create "comment_crdts" table
CREATE TABLE \`comment_crdts\` (
  ...
  yield* sql`-- Create "comment_commits" table
CREATE TABLE \`comment_commits\` (
  ...
  yield* sql`-- Create "comments" table
CREATE TABLE \`comments\` (
  ...
  yield* sql`-- Create "comment_translations" table
CREATE TABLE \`comment_translations\` (
  ...
  yield* sql`-- Create "bookmarks_vegetables" table
CREATE TABLE \`bookmarks_vegetables\` (
  ...
  yield* sql`-- Create "bookmarks_resources" table
