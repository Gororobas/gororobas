...
export class LangExtractService extends Effect.Service<LangExtractService>()("LangExtractService", {
  effect: Effect.gen(function* () {
    ...
    const maxConcurrentExtractions = yield* Config.number("OLLAMA_MAX_CONCURRENT_EXTRACTIONS").pipe(
    ...
    const semaphore = yield* Effect.makeSemaphore(maxConcurrentExtractions)
    ...
    const extractFromHtml = Effect.fn("LangExtractService.extract")(function* (
      ...
      const result = yield* Effect.tryPromise({
        ...
        Effect.retry(
          Schedule.exponential(Duration.millis(100), 2).pipe(Schedule.compose(Schedule.recurs(3))),
        ),
