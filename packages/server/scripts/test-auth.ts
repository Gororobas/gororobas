...
const program = Effect.gen(function* () {
  ...
  const verification = yield* sql`
    ...
    LIMIT 1
  `
  if (EffectArray.isReadonlyArrayEmpty(verification)) {
  ...
  if (EffectArray.isReadonlyArrayEmpty(dbUser)) {
  ...
  if (EffectArray.isReadonlyArrayEmpty(dbSession)) {
  ...
  if (EffectArray.isReadonlyArrayEmpty(sessionAfterSignOut)) {
