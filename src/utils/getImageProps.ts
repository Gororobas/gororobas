...
export function getImageDimensions(
  ...
  const crop = image.crop as SanityImageCrop
  if (crop) {
    const croppedWidth = width * (1 - (crop?.right || 0) - (crop?.left || 0))
 ...
 */
function createGetImageProps(imageBuilder: ImageUrlBuilder) {
  return function getImageProps(props: {
    image: ImageForRendering & { alt?: string }
